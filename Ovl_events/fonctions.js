...
async function dl_save_media_ms(ovl, message) {
  try {
    ...
    console.error("Erreur dl_save_media_ms:", err.message);
    throw err;
  }
}

// --------------------
// Décoder un JID multi-device
// --------------------
const decodeJid = (jid) => {
  if (!jid) return jid;
  if (/:\d+@/gi.test(jid)) {
...
async function recup_msg({ ovl, auteur, ms_org, temps = 30000 } = {}) {
  return new Promise(async (resolve, reject) => {
    ...
    if (temps > 0) {
      timer = setTimeout(() => {
        ovl.ev.off("messages.upsert", listener);
        reject(new Error("Timeout: aucun message reçu dans le temps imparti."));
      }, temps);
    }
  });
}

module.exports = { dl_save_media_ms, recup_msg, decodeJid };
